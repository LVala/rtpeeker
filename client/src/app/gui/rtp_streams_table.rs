...
impl RtpStreamsTable {
    ...
    fn build_table_body(&mut self, body: TableBody) {
        ...
        body.rows(100.0, streams.streams.len(), |id, mut row| {
            ...
            let stream_packets_ids = &streams.streams.get(stream_ssrc).unwrap().rtp_packets;

            let first_packet = streams
                .packets
                .get(*stream_packets_ids.first().unwrap())
                .unwrap();
            let last_packet = streams
                .packets
                .get(*stream_packets_ids.last().unwrap())
                .unwrap();
            ...
            row.col(|ui| {
                ui.label(stream_packets_ids.len().to_string());
            });
            row.col(|ui| {
            ...
            row.col(|ui| {
                let number_of_packets = stream_packets_ids.len() as f64;
            ...
            row.col(|ui| {
                ...
                stream_packets_ids.iter().for_each(|&packet_id| {
                    let packet = streams.packets.get(packet_id).unwrap();
                    if let Some(prev_packet) = previous_packet {
                            ...
                            || rtp.payload_type.id != last_rtp.payload_type.id
                        {
                            ...
                            if let Some(arrival_time_difference) = arrival_time_difference_result {
                                ...
                                let d =
                                    arrival_time_difference.as_secs_f64() - timestamp_difference;
